...
class Shader {
    constructor(gl, vertex_file, fragment_file) {
        ...
        if (!gl.getShaderParameter(vert, gl.COMPILE_STATUS)) {
            alert(`Error compiling shader: ${gl.getShaderInfoLog(vert)}`)
            gl.deleteShader(vert)
        }

        gl.compileShader(frag)
        if (!gl.getShaderParameter(frag, gl.COMPILE_STATUS)) {
            alert(`Error compiling shader: ${gl.getShaderInfoLog(frag)}`)
        ...
        if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            alert(`Error initializing shader: ${gl.getProgramInfoLog(program)}`)
